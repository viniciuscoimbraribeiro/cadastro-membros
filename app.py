...
# --- SEÇÃO IGREJA (Centralizada) ---
    ...
    def renderizar_membro_completo(idx, df_contexto, sufixo):
        ...
        rg_f = re.sub(r'\D', '', val_rg) if val_rg != "nan" else "Não Aplicável"
        ...
        with st.expander(f"👤 {nome_exibicao}"):
            ...
            if not st.session_state[edit_key]:
                ...
                with c2:
                    ...
                    st.write("**Filhos:**")
                    for i in range(1, 4):
                        f_nome = tratar_campo_local(membro.get(f'Nome do Filho (a) - {i}', "Não Aplicável"))
                        if f_nome != "Não Aplicável":
                            st.write(f"👶 {f_nome} ({membro.get(f'Idade do Filho(a) - {i}', 0)} anos)")

                st.divider()
                col_pri, col_ed, col_ex = st.columns(3)
                
                # --- CORREÇÃO DA IMPRESSÃO (CHAVES ESCAPADAS) ---
                if col_pri.button("🖨️ Imprimir Ficha", key=f"btn_prt_{idx}_{sufixo}"):
                    filhos_html = ""
                    for i in range(1,4):
                        fn = tratar_campo_local(membro.get(f'Nome do Filho (a) - {i}', "Não Aplicável"))
                        if fn != "Não Aplicável":
                            fi = membro.get(f'Idade do Filho(a) - {i}', 0)
                            filhos_html += f"<li>{fn} ({fi} anos)</li>"
                    ...
                    <script>
                        ...
                        win.document.write('<div class="section"><b>Filhos:</b><ul>{filhos_html if filhos_html else "<li>Nenhum</li>"}</ul></div>');
                ...
                # --- MODO EDIÇÃO CORRIGIDO (KEYERROR FILHOS) ---
                with st.form(key=f"form_edit_{idx}_{sufixo}"):
                    st.subheader(f"Editando: {membro['Nome Completo']}")
                    ed_nome = st.text_input("Nome", value=membro['Nome Completo'])
                    novos_dados_filhos = {}
                    
                    for i in range(1, 4):
                        # Uso do .get() para evitar KeyError se a coluna não existir no Excel
                        col_nome_f = f'Nome do Filho (a) - {i}'
                        col_nasc_f = f'Data Nascimento Filho {i}'
                        col_idade_f = f'Idade do Filho(a) - {i}'
                        
                        st.write(f"--- Filho {i} ---")
                        novos_dados_filhos[f'nome_{i}'] = st.text_input(f"Nome F{i}", value=tratar_campo_local(membro.get(col_nome_f, "")), key=f"ed_f_n_{i}_{idx}")
                        novos_dados_filhos[f'nasc_{i}'] = membro.get(col_nasc_f, "") 
                        novos_dados_filhos[f'idade_{i}'] = st.number_input(f"Idade F{i}", value=int(membro.get(col_idade_f, 0)), key=f"ed_f_i_{i}_{idx}")
    ...
    with tab_busca:
        nome_busca = st.text_input("Digite o nome", key="input_busca")
        if nome_busca:
            df = conn.read(ttl="5s")
            indices = df[df['Nome Completo'].astype(str).apply(normalizar_local).str.contains(normalizar_local(nome_busca), na=False)].index
            for idx in indices:
